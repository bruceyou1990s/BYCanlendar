...
- (void)initialize
{
    ...
    collectionView.backgroundColor = [UIColor clearColor];//整个view的背景
...
- (CGFloat)preferredWeekdayHeight//week cell 的高度
{
...
- (void)invalidateLayout
{
    if (!self.floatingMode) {
        ...
        if (!_weekdays.count) {  //veryShortWeekdaySymbols 这个属性是一周七天的缩写 跟随系统文字变化  这是是初始化  可以在引用的时候修改属性
            NSArray *weekSymbols = self.calendar.veryShortWeekdaySymbols;
